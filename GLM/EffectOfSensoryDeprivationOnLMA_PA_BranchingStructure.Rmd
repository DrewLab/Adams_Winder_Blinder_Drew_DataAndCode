---
title: "Statistical supplement: Effect of sensory deprivation on LMA, PA, and branching structure"
output:
...
To account for the nested data structure, non-normal distribution, and varying size of cortical regions. We used a generalized linear model (GLIM) based on a Poisson distribution with an offset term which models the logarithm of the count data as a linear combination of a set of independent variables:

(@GLIM1) $$log(C) = X\beta + Zu + \epsilon$$
...
(@GLIM2) $$log(C)=\beta_0+R\beta_1 + \epsilon$$
...
(@GLIM3) $$log(\frac{C}{A}) = \beta'_0 + \beta'_1 R + \epsilon$$

where $A$ was a vector containing the area corresponding to each count in $C$. 

Rearranging:

(@GLIM4) $$log(C) = \beta'_0 + \beta'_1 R + log(A) + \epsilon$$
...
(@GLIM5) $$\beta'_0 = \gamma_1 S + \gamma_2 Y + \gamma_3 G + u_1 M$$
...
(@GLIM6) $$\gamma_2=\alpha_0 + \alpha_1 S$$
...
(@GLIM7) $$\beta'_1 = \gamma_4 + \gamma_5 S$$
...
(@GLIM8) $$log(C) = \gamma_1 S + \alpha_0 Y + \alpha_1(Y*S) + \gamma_3 G + \gamma_4 R + \gamma_5 (S*R) + u_1M + log(A) + \epsilon$$
...
ggplot(data = Data, mapping = aes(x=AvgNumVertInBranch)) +
  ...
  labs(title = "Distribution of all branch densities:", x = "Branches per vessel", 
       y = "Frequency")
```
**Comment**: These data were not normal and were continuous. We therefore used a GLIM based on a Gamma-distribution.
